    ...
    return GreatToken.deployed()
      ...
      .then((totalSupply) => {
        assert.equal(
          ...
          10000000,
          " sets the total supply to 10.000.000  TEST"
        );
      ...
      .then((adminBalance) => {
        assert.equal(
          ...
          10000000,
          "it alocates the inital supply to the admin account"
        );
      });
  });
    ...
    return GreatToken.deployed()
      ...
      .then((receipt) => {
        ...
        assert.equal(
          ...
          "Transfer",
          "first one should be the Transfer event"
        );
        assert.equal(
          receipt.logs[0].args._from,
          accounts[0],
          "logs the account the token are transfered from"
        );
        assert.equal(
          receipt.logs[0].args._to,
          accounts[1],
          "logs the account the token are transfered to"
        );
        assert.equal(
      ...
      .then((balance) => {
        assert.equal(
          balance.toNumber(),
          25000,
          "adds the amont to receiving account"
        );
        return tokenInstance.balanceOf(accounts[0]);
      })
      .then((balance) => {
        assert.equal(
          balance.toNumber(),
          9975000,
          "substract the amount from sending account"
        );
      });
  });
    ...
    return GreatToken.deployed()
      ...
      .then((receipt) => {
        ...
        assert.equal(
          receipt.logs[0].event,
          "Approval",
          "first one should be the Approval event"
        );
        assert.equal(
          receipt.logs[0].args._owner,
          accounts[0],
          "logs the account that tokens are authorized by"
        );
        assert.equal(
          receipt.logs[0].args._spender,
          accounts[1],
          "logs the account the tokens are authotrized to"
        );
        assert.equal(
          receipt.logs[0].args._value,
          100,
  ...
  it("handles delegate tokens transfer", () => {
    return GreatToken.deployed()
      ...
      .catch((error) => {
        assert(
          error.message.indexOf("revert") >= 0,
          "cannot transfer value larger then balance"
        );
